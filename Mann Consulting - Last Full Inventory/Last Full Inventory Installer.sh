#!/bin/zsh
###############################################################################
# Name:     Last Inventory Installer
# Creator:  Mann Consulting
# Summary:  Installer for Mann's Last Full Inventory Workflow
##
...
echo "Uploading computerextensionattributes/Mann Consulting - Last Full Inventory.xml"
curl -sS -H "Authorization: Bearer $authorizationToken" "$jamfpro_url/JSSResource/computerextensionattributes" -H "Content-Type: application/xml" -X "POST" --data-binary "@$baseDir/computerextensionattributes/Mann Consulting - Last Full Inventory.xml"
sleep 20
echo ""

echo "Uploading computergroups/Mann Consulting - Last Full Inventory = Pass.xml"
curl -sS -H "Authorization: Bearer $authorizationToken" "$jamfpro_url/JSSResource/computergroups" -H "Content-Type: application/xml" -X "POST" --data-binary "@$baseDir/computergroups/Mann Consulting - Last Full Inventory = Pass.xml"
sleep 20
echo ""

echo "Uploading computergroups/Mann Consulting - Last Full Inventory = Fail.xml"
curl -sS -H "Authorization: Bearer $authorizationToken" "$jamfpro_url/JSSResource/computergroups" -H "Content-Type: application/xml" -X "POST" --data-binary "@$baseDir/computergroups/Mann Consulting - Last Full Inventory = Fail.xml"
