#!/bin/zsh
###############################################################################
# Name:     Employee Is Active Installer
# Creator:  Mann Consulting
# Summary:  Installer for Mann's Employee Is Active Workflow
##
...
echo "Uploading computerextensionattributes/Mann Consulting - Employee Is Active.xml"
curl -sS -H "Authorization: Bearer $authorizationToken" "$jamfpro_url/JSSResource/computerextensionattributes" -H "Content-Type: application/xml" -X "POST" --data-binary "@$baseDir/computerextensionattributes/Mann Consulting - Employee Is Active.xml"
sleep 20
echo ""

echo "Uploading computergroups/Mann Consulting - Employee Is Active = Pass.xml"
curl -sS -H "Authorization: Bearer $authorizationToken" "$jamfpro_url/JSSResource/computergroups" -H "Content-Type: application/xml" -X "POST" --data-binary "@$baseDir/computergroups/Mann Consulting - Employee Is Active = Pass.xml"
sleep 20
echo ""

echo "Uploading computergroups/Mann Consulting - Employee Is Active = Fail.xml"
curl -sS -H "Authorization: Bearer $authorizationToken" "$jamfpro_url/JSSResource/computergroups" -H "Content-Type: application/xml" -X "POST" --data-binary "@$baseDir/computergroups/Mann Consulting - Employee Is Active = Fail.xml"
